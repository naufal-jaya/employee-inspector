...
function renderImageResults(data) {
  ...
  setText("imgObjectCount", `${allObjects.length} items`);
...
function buildPersonCard(person, index) {
  ...
  const carriedLine = carriedHtml
    ? `<span>Carried (not worn): ${carriedHtml}</span>`
    : "";
  ...
  return `
    <div class="result-card ${isCompliant ? "compliant" : "non-compliant"}">
      ...
      <div class="result-card__meta">
        <span>Detected: ${presentHtml}</span>
...
async function startLiveCamera() {
  ...
  if (!navigator.mediaDevices?.getUserMedia) {
    showStatus("liveStatusMsg", "Camera API is not supported in this browser.", true);
    return;
  }
  try {
